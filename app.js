...
let requiredStandard = { label: 'Target: 120 kWh/m²', maxDemandKwh: 120 };
...
async function loadSelectedApartment(selection) {
    ...
    try {
        ...
        activePhysics = result.data.physics; const activePropertyType = (result.data.property_type || 'house').toLowerCase(); gridCapacity = result.data.grid?.headroom_pct ?? 88; isGridEstimate = Boolean(result.data.grid?.estimated); winterTemp = result.data.weather?.winter_design_temp ?? -3.8; requiredStandard = { label: result.data.standard?.label || 'Target: 120 kWh/m²', maxDemandKwh: result.data.standard?.max_demand_kwh ?? 120 };
        const sources = result.data.sources || {};
        document.getElementById('designTemp').textContent = `${winterTemp}°C`; document.getElementById('gridHeadroom').textContent = isGridEstimate ? `${gridCapacity}%*` : `${gridCapacity}%`; document.getElementById('rainExposure').textContent = result.data.weather?.rain_exposure ? `${result.data.weather.rain_exposure} (${result.data.weather.annual_rainfall_mm} mm/yr)` : '--';
        ...
        gridSourceLabel = sources.grid?.provider || (isGridEstimate ? 'Grid Estimate' : 'DNO Open Data'); const gridSourceLabelEl = document.getElementById('gridSourceLabel'); if (gridSourceLabelEl) gridSourceLabelEl.textContent = gridSourceLabel;
    ...
    } catch (error) { console.error('Load error:', error); alert(`API Error: ${error.message}`); epcMetaEl.textContent = `Property load failed: ${error.message}`; hrrScoreEl.textContent = 'ERR'; }
}

function calculateMaxPotential() { if (!activePhysics || !activePhysics.osFloorArea) return; const { volume, wallArea, roofArea, windowArea, finalACH, osFloorArea } = activePhysics; const minDemand = Math.round((((volume * finalACH * 0.33) + (wallArea * 0.3) + (roofArea * 0.11) + (windowArea * 2.0)) * 2500 * 24 * 0.75) / 1000 / osFloorArea); const bestBand = minDemand <= 80 ? 'B' : (minDemand <= 120 ? 'C' : 'D'); epcMetaEl.textContent = `Maximum Achievable Potential: ${minDemand} kWh/m² (Band ${bestBand})`; }
function recalculateSandbox() { if (!activePhysics || !activePhysics.osFloorArea) return; let { volume, wallArea, roofArea, windowArea, uWall, uRoof, finalACH, osFloorArea } = activePhysics; const toggleLoft = document.getElementById('toggleLoft'); const toggleIWI = document.getElementById('toggleIWI'); const toggleEWI = document.getElementById('toggleEWI'); if (toggleLoft && toggleLoft.checked && roofArea > 0) uRoof = 0.11; if ((toggleIWI && toggleIWI.checked) || (toggleEWI && toggleEWI.checked)) uWall = 0.3; const newHTC = (volume * finalACH * 0.33) + (wallArea * uWall) + (roofArea * uRoof) + (windowArea * 2.0); const currentDemand = Math.round((newHTC * 2500 * 24 * 0.75) / 1000 / osFloorArea); updateUI(currentDemand); updateCharts(currentDemand, newHTC); }
function updateUI(demand) { const target = requiredStandard.maxDemandKwh || 120; const isPass = demand <= 120; const currentBand = demand <= 80 ? 'B' : (demand <= 120 ? 'C' : 'D'); hrrScoreEl.textContent = currentBand; heatDemandEl.textContent = `${demand} kWh/m²`; const heetsaBandEl = document.getElementById('heetsaBand'); if (heetsaBandEl) heetsaBandEl.textContent = `Band ${currentBand} (${demand} kWh/m²)`; const complianceEl = document.getElementById('standardCompliance'); if (complianceEl) { complianceEl.textContent = isPass ? '✓ Meets standard' : '✗ Does not meet standard'; complianceEl.classList.remove('compliance-pass', 'compliance-fail'); complianceEl.classList.add(isPass ? 'compliance-pass' : 'compliance-fail'); } dialContainer.classList.remove('dial-pass', 'dial-fail'); dialContainer.classList.add(isPass ? 'dial-pass' : 'dial-fail'); hrrScoreEl.classList.toggle('text-fail', !isPass); heatDemandEl.classList.toggle('text-fail', !isPass); if (demand > 70) { lockHeatPump('🔒 Requires Demand ≤ 70 kWh/m²'); } else if (gridCapacity < 85) { lockHeatPump(`🔒 ${gridSourceLabel} Constrained (${gridCapacity}% Cap)`); } else { toggleASHP.disabled = false; toggleASHP.closest('.switch').classList.remove('disabled-switch'); ashpGatekeeper.textContent = '✓ Thresholds met'; ashpGatekeeper.style.color = 'var(--accent-green)'; } }
