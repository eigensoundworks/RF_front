...
async function loadSelectedApartment(selection) {
    ...
    try {
        ...
        document.getElementById('designTemp').textContent = winterTemp === null ? 'Unavailable' : `${formatNumber(winterTemp)}°C`; document.getElementById('gridHeadroom').textContent = gridCapacity === null ? 'Unavailable' : (isGridEstimate ? `${formatNumber(gridCapacity)}%*` : `${formatNumber(gridCapacity)}%`); document.getElementById('rainExposure').textContent = result.data.weather?.rain_exposure ? `${result.data.weather.rain_exposure} (${formatNumber(result.data.weather.annual_rainfall_mm)} mm/yr)` : '--';
        ...
        const standardLabelEl = document.getElementById('standardLabel'); if (standardLabelEl) standardLabelEl.textContent = `Target: ${formatNumber(requiredStandard.maxDemandKwh)} kWh/m²`;
        const requiredStandardEl = document.getElementById('requiredStandard'); if (requiredStandardEl) requiredStandardEl.textContent = `${formatNumber(requiredStandard.maxDemandKwh)} kWh/m²`;
...
function calculateMaxPotential() { if (!activePhysics || !activePhysics.osFloorArea || !activePhysics.annualHdd) return; const { volume, wallArea, roofArea, windowArea, finalACH, osFloorArea, annualHdd } = activePhysics; const minDemand = Math.round((((volume * finalACH * 0.33) + (wallArea * 0.3) + (roofArea * 0.11) + (windowArea * 2.0)) * annualHdd * 24) / 1000 / osFloorArea); const target = requiredStandard.maxDemandKwh || 120; const isPass = minDemand <= target; epcMetaEl.textContent = `Maximum Achievable Potential: ${formatNumber(minDemand)} kWh/m² (${isPass ? 'PASS' : 'FAIL'})`; }
...
function updateUI(demand) { const target = requiredStandard.maxDemandKwh || 120; const isPass = demand <= target; hrrScoreEl.textContent = isPass ? 'PASS' : 'FAIL'; heatDemandEl.textContent = `${formatNumber(demand)} kWh/m²`; const heetsaBandEl = document.getElementById('heetsaBand'); if (heetsaBandEl) heetsaBandEl.textContent = `${isPass ? 'PASS' : 'FAIL'} (${formatNumber(demand)} kWh/m²)`; const complianceEl = document.getElementById('standardCompliance'); if (complianceEl) { complianceEl.textContent = isPass ? '✓ Meets standard' : '✗ Does not meet standard'; complianceEl.classList.remove('compliance-pass', 'compliance-fail'); complianceEl.classList.add(isPass ? 'compliance-pass' : 'compliance-fail'); } dialContainer.classList.remove('dial-pass', 'dial-fail'); dialContainer.classList.add(isPass ? 'dial-pass' : 'dial-fail'); hrrScoreEl.classList.toggle('text-fail', !isPass); heatDemandEl.classList.toggle('text-fail', !isPass); if (demand > 70) { lockHeatPump('🔒 Requires Demand ≤ 70 kWh/m²'); } else if (gridCapacity === null) { lockHeatPump(`🔒 ${gridSourceLabel} Unavailable`); } else if (gridCapacity < 85) { lockHeatPump(`🔒 ${gridSourceLabel} Constrained (${formatNumber(gridCapacity)}% Cap)`); } else { toggleASHP.disabled = false; toggleASHP.closest('.switch').classList.remove('disabled-switch'); ashpGatekeeper.textContent = '✓ Thresholds met'; ashpGatekeeper.style.color = 'var(--accent-green)'; } }
function updateCharts(demand, newHTC) { if (!activePhysics || !activePhysics.osFloorArea) return; const valWinter = document.getElementById('valWinter'); const valSummer = document.getElementById('valSummer'); const barWinter = document.getElementById('barWinter'); const barSummer = document.getElementById('barSummer'); if (!valWinter || !valSummer || !barWinter || !barSummer) return; const floorArea = activePhysics.osFloorArea; const winterAnnual = Math.round(Math.max(0, demand) * floorArea); const summerRisk = Math.round(Math.max(0, newHTC * 24 * 45 * 0.06)); valWinter.textContent = formatNumber(winterAnnual); valSummer.textContent = formatNumber(summerRisk); const winterPct = Math.min(100, Math.max(0, Math.round((winterAnnual / 24000) * 100))); const summerPct = Math.min(100, Math.max(0, Math.round((summerRisk / 10000) * 100))); barWinter.style.width = `${winterPct}%`; barSummer.style.width = `${summerPct}%`; }
